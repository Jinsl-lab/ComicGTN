...
from .utils import Norm_
from .utils import Generate_Non_Local_Graph
from .GCNConv import *


class FastGTN(nn.Module):
    def __init__(self, num_edge_type, w_in, num_nodes, args = None, pre_trained = None):
        ...
        # Initialize each layer using the pre-trained parameters pre_trained.
        else:
            layers = []
            for i in range(self.num_layers):
                if i == 0:
                    layers.append(FastGTLayer(num_edge_type, self.num_channels, num_nodes, first = True, 
                                                                args = args, pre_trained = pre_trained[i]))
                else:
                    layers.append(FastGTLayer(num_edge_type, self.num_channels, num_nodes, first = False, 
                                                                args = args, pre_trained = pre_trained[i]))
            self.layers = nn.ModuleList(layers)
    ...
    def forward(self, A, X, num_nodes, epoch = None):
        ...
        for i in range(self.num_layers):
            ...
            if self.args.non_local:
                g = Generate_Non_Local_Graph(self.args, self.feat_trans_layers[i], torch.stack(H).mean(dim = 0), A,
                                                                    self.num_edge_type, num_nodes)
                deg_inv_sqrt, deg_row, deg_col = Norm_(g[0].detach(), num_nodes, g[1])
            ...
            # Call FastGTLayer for message passing and feature updates.
            H, W = self.layers[i](H, A, num_nodes, epoch = epoch, layer = i + 1)
            Ws.append(W)

        # Aggregate features from multiple channels.
        for i in range(self.num_channels):
            if i == 0:
...
class FastGTLayer(nn.Module):
    ...
    def forward(self, H_, A, num_nodes, epoch = None, layer = None):
        ...
        # Call FastGTConv to generate a new graph structure result_A.
        result_A, W1 = self.conv1(A, num_nodes, epoch = epoch, layer = layer)
        W = [W1]
        Hs = []
        
        # Perform message passing on the new graph structure to update node features Hs.
...
class FastGTConv(nn.Module):
    ...
    def forward(self, A, num_nodes, epoch = None, layer = None):
        ...
        # Perform a weighted summation on the input adjacency matrix list A to generate a new graph structure results.
